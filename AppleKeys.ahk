...
Loop %Count% {
	Handle:=NumGet(RawInputList, (A_Index-1)*cfg.SizeofRawInputDeviceList), Type:=NumGet(RawInputList, (A_Index-1)*cfg.SizeofRawInputDeviceList+4)
	...
	VarSetCapacity(Info, cfg.SizeofRidDeviceInfo), NumPut(cfg.SizeofRidDeviceInfo, Info, 0)
	Length:=cfg.SizeofRidDeviceInfo, Res:=DllCall("GetRawInputDeviceInfo", UInt, Handle, UInt, cfg.RIDI_DEVICEINFO, UInt, &Info, "UInt *", Length)
	
	; AWK modifier buttons is separate TYPEHID (rather keyboard standard buttons)
	if (Type=cfg.RIM_TYPEHID)
		Vendor:=NumGet(Info, 4*2, "UShort"), Product:=NumGet(Info, 4*3, "UShort"), Version:=NumGet(Info, 4*4, "UShort"), UsagePage:=NumGet(Info, (4*5), "UShort"), Usage:=NumGet(Info, (4*5)+2, "UShort")
	...
	if (Type=cfg.RIM_TYPEHID && Vendor=1452  && cfg.rimHIDregistered=0) {
		cfg.rimHIDregistered:=1, NumPut(UsagePage, RawDevice, 0, "UShort"), NumPut(Usage, RawDevice, 2, "UShort"), Res:=DllCall("RegisterRawInputDevices", "UInt", &RawDevice, UInt, 1, UInt, cfg.SizeofRawInputDevice)
		if (Res=0) {
